...
const DOMAIN_SOURCES = {
  ...
  // BanMing fork splits asset ops across two tools: FMCPTool_Asset holds the
  // property/save/get/list ops; FMCPTool_AssetManage holds search/find/CRUD
  // (the JS router transparently sub-routes via ASSET_MANAGE_OPS). The parity
  // scan must read BOTH source files or every CRUD op shows up as orphan.
  asset: ["MCPTool_Asset.cpp", "MCPTool_AssetManage.cpp"],
  blueprint: [
...
function extractRouterOpsForDomain(domain) {
  ...
  for (let i = startIdx + 1; i < lines.length; i++) {
    ...
    // Strip the leading label. Supports upstream's plain "ops:" / "modify ops:" /
    // "query ops:" plus BanMing's qualified "property ops (FMCPTool_Asset):" /
    // "CRUD ops (FMCPTool_AssetManage):" form for the split-tool asset domain.
    const cleaned = line.replace(
      /^\s*(modify ops:|query ops:|ops:|[a-z]+ ops \([^)]*\):)?\s*/i,
      ""
    );
